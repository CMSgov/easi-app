...
// GetContractBySystem queries CEDAR for contract information (NJD EXPAND ON THIS?) associated with a particular system, taking the version-independent ID of a system
func (c *Client) GetContractBySystem(ctx context.Context, cedarSystemID string) ([]*models.CedarContract, error) {
	...
	for _, contract := range resp.Payload.Contracts {
		...
		retVal = append(retVal, &models.CedarContract{
			...
			ContractAdo:           &contract.ContractADO,
			ContractDeliverableID: &contract.ContractDeliverableID,
